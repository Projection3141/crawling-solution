...
function getProductImages(detailItem, productItem) {
  ...
  const detailImagesObject =
    detailItem?.images && typeof detailItem.images === "object"
      ...
      console.log("\ndetailImagesObject : ",detailImagesObject);
...
//   const baseItems = isDetail && detailItems.length > 0
//     ? detailItems
//     : products;
...
//   /** 목록에서 누락됐지만 실제 재고에 존재하는 상품도 보존한다. */
//   for (const productId of inventoryMap.keys()) {
//     if (seenIds.has(productId)) {
//       continue;
//     }
...
//       translationItem?.nameEn,
//     );
//     const nameJa = isDetail
//       ? ""
//       : normalizeText(translationItem?.nameJa);
